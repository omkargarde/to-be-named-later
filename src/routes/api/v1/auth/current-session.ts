...
export const Route = createFileRoute("/api/v1/auth/current-session")({
  server: {
    middleware: [requestLogger],
    handlers: {
      POST: async function ({ request }) {
        console.log("[current_session] validating request");
        ...
        if (!parsed.success) {
          console.error("[current_session] bad request returning error response");
